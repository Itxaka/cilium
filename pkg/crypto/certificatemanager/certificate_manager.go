...
type k8sClient interface {
	GetSecrets(ns, name string) (map[string][]byte, error)
}
...
func (m *Manager) GetTLSContext(tlsCtx *api.TLSContext) (ca, public, private string, err error) {
	...
	if tlsCtx.K8sSecret != nil {
		...
		secrets, k8sErr := m.k8sClient.GetSecrets(ns, name)
		if k8sErr != nil {
			return "", "", "", k8sErr
		}
